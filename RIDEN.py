            ...
            if msg.contentType == 1:
              if Squad['Upfoto'] == True:
                if user in Owner:
                    ...
                    cl.mentionWithRFU(kirim,owner," Update Picture Success ","")
                    Squad['Upfoto'] = False
...
#======= AUTO TAG & CHAT BATAS SCRIP ========
        ...
        if fast.type == 26:
            ...
            user = msg._from
            kirim = msg.to
            if msg.contentType == 0 and user not in RfuSekawan and msg.toType == 2:
                if "MENTION" in msg.contentMetadata.keys() != None:
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                gpl = cl.getGroupIdsJoined()
                                gpl = riden1.getGroupIdsJoined()
                                gpl = riden2.getGroupIdsJoined()
                                gpl = riden3.getGroupIdsJoined()
                                for all in gpl:
                                    cl.leaveGroup(all)
                                    riden1.leaveGroup(all)
                                    riden2.leaveGroup(all)
                                    riden3.leaveGroup(all)
                          ...
                          if user in RfuSekawan:
                            ...
                            if user in RfuSekawan:
                               ...
                               for target in targets:
                                   try:
                                       contact = cl.getContact(target)
                                       cl.sendMessage(kirim, None, contentMetadata={'mid': target.mid}, contentType=13)
                                       cl.sendText(kirim,"Nama :\n" + msg.contentMetadata["displayName"] + "\n\nBio :\n" + contact.statusMessage+ "\n\nMid :\n" + msg.contentMetadata["mid"] + "\n\nSteal Succes..")
                                       image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                                       cl.sendImageWithURL(kirim,image)
                                       cover = cl.getProfileCoverURL(target)
                                       cl.sendImageWithURL(kirim, cover)
                                       cl.sendText(kirim, str(target.mid))
                                   except:
                                       cl.sendText(kirim,"Contact Error")
